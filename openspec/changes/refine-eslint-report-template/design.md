...
- Before every checker run, the workflow must delete any existing `.eslint-checker` directory in the business project root so the report is generated from a complete fresh run.
...
All report-facing non-technical prose should be Chinese. Technical terms and keywords such as `ESLint`, `Vue`, `React`, `TypeScript`, `JavaScript`, `Node`, `npm`, `report.json`, `lint script`, `riskAssessment`, `eslintConfigAnalysis`, `ruleId`, command strings, file paths, package names, and artifact names may remain English.
...
Each report run must start by checking whether `.eslint-checker` already exists in the business project root. If it exists, delete the whole directory before running `npx @sunny/eslint-checker --mode full --for-iflycode`.

The workflow must not reuse an old `.eslint-checker/report.json`, old `.eslint-checker/summary.md`, old `.eslint-checker/iflycode-key-data.json`, or old `.eslint-checker/iflycode-eslint-governance-report.md`.
...
The report generator must translate `report.json` display values before writing `.eslint-checker/iflycode-eslint-governance-report.md`.
